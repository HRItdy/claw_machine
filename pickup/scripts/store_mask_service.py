...
def handle_store_mask(req):
    ...
    if req.store:
        ...
        if stored_mask is not None:
            ...
            response_mask.layout.dim = [
                Int32MultiArray.MultiArrayDimension(size=stored_mask.shape[0], stride=stored_mask.size, label="rows")
            ]
...
def store_mask_client(mask=None, store=True):
        ...
        if mask is not None:
            ...
            array_msg.layout.dim = [Int32MultiArray.MultiArrayDimension(size=mask.shape[0], stride=mask.size, label="rows")]
        resp = store_mask(store, array_msg)
        return np.array(resp.stored_mask.data).reshape(resp.stored_mask.layout.dim[0].size, -1), resp.success
