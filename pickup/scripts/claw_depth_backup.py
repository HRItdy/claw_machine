...
class ClawDepth:
    def __init__(self):
        ...
        self.pc_pub_seg = rospy.Publisher('/seg_point_cloud', PointCloud, queue_size=1)
    ...
    def callback(self, depth_msg, color_msg, depth_info, color_info):
        ...
        source_cloud = self.color_to_point_cloud(self.depth_image, self.depth_intrinsics, color_image=self.color_image)
        ...
        with open('output_mask.txt', 'w') as file:
            for row in indices:
                # Convert each item in the row to a string and join them with spaces or commas
                file.write(' '.join(map(str, row)) + '\n')
        seg_pc = self.points_to_point_cloud(self.depth_image, self.depth_intrinsics, indices=indices)
        self.cluster_pub(seg_pc, self.pc_pub_seg)
    ...
    def color_to_point_cloud(self, depth_image, intrinsics, color_image):
    ...
    def points_to_point_cloud(self, depth_image, intrinsics, indices):
        point_cloud = []
        u_list = [indice[1] for indice in indices]
        v_list = [indice[0] for indice in indices]
        for v in v_list:
            for u in u_list:
                if depth_image[v, u] > 0:
                    point = rs2.rs2_deproject_pixel_to_point(intrinsics, [u, v], float(depth_image[v, u]) * 0.001)
